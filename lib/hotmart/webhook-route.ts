...
/**
 * Handler compartilhado por /api/webhooks/hotmart e /api/webhook.
 */
export async function processHotmartWebhook(request: Request): Promise<NextResponse> {
  const ip =
  ...
  const expectedHottok = process.env.HOTMART_HOTTOK || process.env.HOTTOK || null;
    ...
    return NextResponse.json(
      {
        ok: false,
        acknowledged: true,
        error: "Evento recebido; falha interna no processamento.",
      },
      { status: 200 }
    );
  }
